...
This is a static React website. It does not use login, a database, personal-data storage or a server-side API. Navigation uses URL hashes (for example `#/map`) so all thirteen pages work on ordinary static hosting without special route-rewrite rules.
